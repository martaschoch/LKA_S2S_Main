...
use "$hies/RAW/LKA_2016_HIES_v01_M", clear 
tostring person_serial_no, replace
...
merge 1:1 pid hhid using "$hies/SARMD/HIES_2016" , nogen keepusing(welfare subnatid*)

// Merge non-labor income components from harmonized file 
merge 1:1 pid hhid using "$hies/SARMD/HIES_2016_inc_hh"  , nogen 
...
// Firewood/Water 
*******************************************************************************
gen distance_to_water  	= water_distance
...
********************************************************************************
xtile decile = welfare [aw=weight] , nq(10)
...
foreach var in pensions capital remittances inocct_m {
	...
	gen r`var'pc = ((hh_`var'_pc * avg_cpi)/cpi_base2013)*1
	su r`var'pc, d 
}

//Deflate Total income per capita: from SARMD
gen ripcfpc = ((ipcf * avg_cpi)/cpi_base2013)*1
su ripcfpc, d 

tabstat welfare hh_inc_primary_nc_pc rpcinc* cpi_base2013, by(month)

xtile quintile19 = welfare [aw=weight] , nq(5)
...
mean dist_* sector_* sin buddhist male age married hhsize  noedu atleast_sec schoolage_noschool  cellphone computer [aw=weight]
