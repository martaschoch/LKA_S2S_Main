...
# -----------------------------
# Step 3: Predictions via PMM in 2022 (HHs w/o income)
# -----------------------------
...
write.csv(tab1,file=paste(outpath,
       "/Outputs/Main/Tables/Poverty_imputed_2022.csv",sep=""),
      row.names = FALSE)

write_dta(lfs.imp,paste(datapath,
       "/lfs2022_imputed.dta",sep=""))
...
write.csv(df.match,file=paste(datapath,
         "cleaned/Stage 2/Final/Simulations_welf_2022_",sim,".csv",sep=""),
          row.names = FALSE)
saveRDS(df.match,file=paste(datapath,
        "cleaned/Stage 2/Final/Simulations_welf_2022_",sim,".rds",sep=""))
