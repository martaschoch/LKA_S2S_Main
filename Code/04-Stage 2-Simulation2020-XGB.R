...
write.csv(tab1,file=paste(outpath,
       "/Outputs/Main/Tables/Poverty_imputed_2020.csv",sep=""),
      row.names = FALSE)

write_dta(lfs.imp,paste(datapath,
       "/lfs2020_imputed.dta",sep=""))
...
write.csv(df.match,file=paste(datapath,
         "cleaned/Stage 2/Final/Simulations_welf_2020_",sim,".csv",sep=""),
          row.names = FALSE)
saveRDS(df.match,file=paste(datapath,
        "cleaned/Stage 2/Final/Simulations_welf_2020_",sim,".rds",sep=""))
