...
# -----------------------------
# Step 3: Predictions via PMM in 2021 (HHs w/o income)
# -----------------------------
...
write.csv(tab1,file=paste(outpath,
       "/Outputs/Main/Tables/Poverty_imputed_2021.csv",sep=""),
      row.names = FALSE)

write_dta(lfs.imp,paste(datapath,
       "/lfs2021_imputed.dta",sep=""))
...
write.csv(df.match,file=paste(datapath,
         "cleaned/Stage 2/Final/Simulations_welf_2021_",sim,".csv",sep=""),
          row.names = FALSE)
saveRDS(df.match,file=paste(datapath,
        "cleaned/Stage 2/Final/Simulations_welf_2021_",sim,".rds",sep=""))
