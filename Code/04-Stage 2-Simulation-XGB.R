...
# (donor, trained on HHs w/o income) and 2024 (receiver, HHS w/o income)
...
hies.don=read_dta(paste(datapath,"cleaned/New bases Stats Team/hies2019_clean.dta",sep="")) 
...
lfs.rec=read_dta(paste(datapath,
                       "cleaned/New bases Stats Team/lfs",year,"_clean.dta",
                       sep="")) 
...
foreach(sim = 1:nsim2) %do% {
    cat("Simulation for year ", year, " HHS with labor income: ", sim, "\n")
    ...
    fA.wrnd$y_nl = with(fA.wrnd, ratio_tot*rpcinc_tot)
...
# -----------------------------
# Step 3: Predictions via PMM in 2020 (HHs w/o income)
# -----------------------------
...
foreach(sim = 1:nsim2) %do% {
    cat("Simulation for year ", year, " HHS w/o labor income: ",sim, "\n")
    # Bootstrap the training data
    ...
    cv_model <- xgb.cv(
       ...
       best_nrounds <- cv_model$evaluation_log[which.min(cv_model$evaluation_log$test_rmse_mean), iter]
        if (is.null(best_nrounds) || length(best_nrounds) == 0 || best_nrounds == 0) {
        best_nrounds <- 100L
        }
    
    # Train model on the bootstrap sample
    model <- xgb.train(params = best_params_1, 
        data = dtrain, nrounds = best_nrounds, verbose = 0)
    ...
    fA.wrnd <- create.fused(data.rec=samp.btemp, data.don=samp.atemp,
                            ...
                            z.vars=don.vars2.1) 
    fA.wrnd.c = fA.wrnd[,c("hhid","welfare")]
    fA.wrnd.i = fA.wrnd[,c("hhid","welfare")] # drop this later, remove all y_nl
    names(fA.wrnd.c)[2]=paste("welfare_",sim,sep="")
    names(fA.wrnd.i)[2]=paste("y_nl_",sim,sep="")
    simcons_match=merge(simcons_match,fA.wrnd.c,by="hhid")
    simcons_match_i=merge(simcons_match_i,fA.wrnd.i,by="hhid")
...
growth_rate = hfce$pct_change_vs_2019[hfce$year == year]

lfs.imp.1 = lfs.imp.1 %>%
  rename(y_nl=y_nl_median) %>%
  mutate(welfare=welfare_median*(1+growth_rate),
    logwelfare=log(welfare))
...
cat("Gini coefficient in simulated data is: ",gini)
...
write.csv(tab1,file=paste(outpath,
       "/Outputs/Main/Tables/Poverty_imputed_",year,".csv",sep=""),
      row.names = FALSE)

write_dta(lfs.imp,paste(datapath,
       "/lfs",year,"_imputed.dta",sep=""))
...
write.csv(df.match,file=paste(datapath,
         "cleaned/Stage 2/Final/Simulations_welf_",year,"_",sim,".csv",sep=""),
          row.names = FALSE)
saveRDS(df.match,file=paste(datapath,
        "cleaned/Stage 2/Final/Simulations_welf_",year,"_",sim,".rds",sep=""))
